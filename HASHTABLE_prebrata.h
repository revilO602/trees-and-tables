//Hashtable with open addressing taken from:
	...
	int_set_create(void)
{
	...
	set = (struct int_set*) malloc(sizeof(*set));
	if (set == NULL)
		return NULL;
	...
	set->table = (struct int_set_entry*)calloc(set->size, sizeof(*set->table));
	set->entries = 0;
...
int_set_rehash(struct int_set* set, int new_size_index)
{
	...
	table = (struct int_set_entry*) calloc(hash_sizes[new_size_index].size, sizeof(*set->table));
	if (table == NULL)
		return;
